...
/* IMPLEMENTATION DE LA CLASSE SVGFILE */
...
Svgfile::~Svgfile()
{
 ...
 int Svgfile:: getLargeur()
 {
    return m_width;
 }

int Svgfile :: getHauteur()
{
    return m_height;
}
...
void Svgfile :: addEllipses(double x, double y, double rx, double ry, Couleur remplissage, Couleur contour , double ep)
{
    m_ostrm << "<ellipse "
            << attrib("cx", x)
            << attrib("cy", y)
            << attrib("rx",  rx)
            << attrib("ry",  ry)
             << attrib("fill", (std::string)remplissage)
            << attrib("opacity", (double)remplissage.getOpacity())
            << attrib("stroke", (std::string)contour )
...
void Svgfile :: addCourbeBezier(int startX, int startY, int point1x, int point1y,int point2x, int point2y, int endx, int endy, std :: string color)
{
    m_ostrm << "<path d=\"M"
            << startX << "," << startY << " "
            << "C" << point1x<< "," << point1y<<" "
